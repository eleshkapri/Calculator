...
## ✨ Features & Separated Calculators

1. 🔢 **Standard Calculator**: Fast arithmetic calculations, bracket parsing, percentages, and memory operations ($MC, MR, M+, M-, MS$).
2. 🔬 **Scientific Calculator**: Trigonometric functions ($\sin, \cos, \tan$, inverses), logarithmic functions ($\ln, \log_{10}$), exponents, factorials ($n!$), constants ($\pi, e$), and Degree/Radian modes.
3. 📈 **Graphing Calculator**: Interactive HTML5 canvas graph plotter for math functions ($y = f(x)$), multi-curve overlay, quick presets, zoom controls, coordinate hover tracker, and drag-to-pan.
4. 💰 **Financial Calculator**:
   - **Loan / EMI Calculator**: Monthly installments, total interest, and visual amortization breakdown with interactive sliders.
   - **Compound Interest & SIP**: Future value projections, total invested vs interest earned.
5. 🔄 **Unit Converter**: Dual-way real-time converter supporting Length, Weight/Mass, Temperature, Area, Speed, Digital Data Storage, and Time.
